...
public class Percolation_Weighted {
    ...
    public Percolation_Weighted(int n) {
        ...
        unionTree = new WeightedQuickUnionUF(n * n);
    ...
    public boolean percolates() {
        for (int i = 0; i <= matrixLength; i++) {
            ...
            for (int j = 0; j <= matrixLength; j++) {
                // Check by column if the index is
                if (unionTree.find(getIndex(0, i)) == unionTree.find(getIndex(matrixLength,))) {
    ...
    public static void main(String[] args) {
        Percolation_Weighted testPer = new Percolation_Weighted(4);
        testPer.percolates();
        testPer.open(0,1);
        testPer.open(1,1);
        testPer.open(2,1);
